...
import { IS_DUMMY_MODE, getDummyUsers, findDummyUserById } from "@/lib/dummy-data"
...
export async function GET() {
  if (IS_DUMMY_MODE) {
    const owners = getDummyUsers().filter((u) => u.role === "owner").map((u) => ({ id: u.id, name: u.name }))
...
export async function POST(request: Request) {
  ...
  if (IS_DUMMY_MODE) {
    const user = findDummyUserById(userId)
    if (!user || user.role !== "owner") return NextResponse.json({ error: "User tidak ditemukan" }, { status: 404 })
