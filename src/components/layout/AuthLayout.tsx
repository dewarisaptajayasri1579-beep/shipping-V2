...
export const AuthLayout: React.FC<AuthLayoutProps> = ({ quickLogin = false, isDummyMode = false, mode = "login" }) => {
  const [showQuickLogin, setShowQuickLogin] = useState(quickLogin);
  const form = (showTopLockIcon: boolean) =>
    mode === "register" ? (
      <RegisterForm showTopLockIcon={showTopLockIcon} isDummyMode={isDummyMode} />
    ) : (
      <LoginForm showTopLockIcon={showTopLockIcon} isDummyMode={isDummyMode} />
    );
